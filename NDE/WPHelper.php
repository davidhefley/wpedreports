	...
	class WPHelper {
				...
				foreach ( $status as $statii ) {
					switch ( $statii ) {
						case 'meets':
							...
							$statusFilter[] = '(`gateway_1`="meets" AND `gateway_2`="meets")';
							break;
						case 'partially-meets':
							...
							$statusFilter[] = '( (`gateway_1`="partially-meets" OR `gateway_2`="partially-meets") AND (`gateway_1`<>"does-not-meet" AND `gateway_2`<>"does-not-meet") )';
							break;
						case 'does-not-meet':
							...
							$statusFilter[] = '(`gateway_1`="does-not-meet" OR `gateway_2`="does-not-meet")';
							break;
		...
		public function series( int $subject = null, array $grades = [], string $text = '', string $state = 'none', array $status = [], string $type = '' ) {
			...
			if ( ! empty( $status ) && count( $status ) != 3 ) { //if there are 3 items, we don't need this filter either!
				$status_ids = $this->getSeriesFromStatus( $status, $type );
				if ( ! empty( $status_ids ) ) {
		...
		private function getSeriesFromStatus( $status = [], $type='textbook' ) {
			global $wpdb;
		...
		private function getSeriesFromBookType( string $type = 'textbook', int $subject_id = null ) {
			if ( empty( $type ) ) {
				return [];
			}
			global $wpdb;
			$reports_table = $wpdb->prefix . 'EdRep_reports';
			$qry           = "SELECT DISTINCT series_id FROM {$reports_table} WHERE `book_type` LIKE \"$type\"";
			if ( !empty($subject_id) ) $qry.=" AND `subject_taxonomy_id` = $subject_id";
			$qry .= ';';
		...
		private function getGradeIds( $grades ) {
			...
			foreach ( $dashCollections as $dash ) {
				...
				if ( count( $gradeBand ) != 2 ) {
					continue;
				} // something went wrong
				foreach ( $grades as $grade ) {
					if ( $grade >= $gradeBand[0] && $grade <= $gradeBand[1] ) {
						$results[] = $dash->id;
					}
				}
			}
		...
		public function updateReportDetails( $limit = 5, $id = '' ) {
			...
			if ( ! empty( $id ) ) {
				...
				$result = $wpdb->replace( $details_table, [ 'id'        => $id,
		...
		public function updateSeriesDetail( $limit = 5, $id = '' ) {
			...
			if ( ! empty( $id ) ) {
				...
				if ( ! empty( $insertData ) ) {
					$singular_insert_data = current( $insertData );
				}
		...
		private function getLastSyncTime( string $name ) {
			$this->loadSyncDates();
			if ( isset( $this->syncDates[ $name ] ) ) {
				return $this->syncDates[ $name ];
			} else {
				return '0000-00-00';
			}
		}
		...
		private function loadSyncDates() {
			if ( $this->syncDates === '' ) {
				...
				if ( empty( $syncdates ) ) {
					$this->syncDates = [];
				} //change it to an array so we know it is loaded
				else {
		...
		private function processAllData( string $table_name, $data ): bool {
			...
			foreach ( $sanitizedData as $insertData ) {
				...
				if ( $result ) {
					$this->debugLog( "Inserted {$result} items." );
				} else {
					$this->debugLog( 'Failed to insert item' );
				}
			}
		...
		private function processUpdates( string $table_name, $updated, $deleted ): bool {
			...
			if ( empty( $sanitizedUpdated ) ) {
				...
				foreach ( $sanitizedUpdated as $updateData ) {
					...
					if ( $result ) {
						$this->debugLog( "Updated {$result} items." );
					} else {
						$this->debugLog( 'Failed to update item' );
					}
				}
			}
			...
			if ( empty( $sanitizedDeleted ) ) {
				...
				foreach ( $sanitizedDeleted as $deleteData ) {
					...
					if ( $result ) {
						$this->debugLog( "Deleted {$result} items." );
					} else {
						$this->debugLog( 'Failed to delete item' );
					}
				}
			}
		...
		public function deleteReportDetail( $id = '' ) {
			...
			global $wpdb;
			$details_table = $wpdb->prefix . 'EdRep_report_details';
			$qry           = "DELETE FROM {$details_table} WHERE id=" . (int) $id . " LIMIT 1;";
			$wpdb->query( $qry );
		}
		...
		public function updatePublishers( int $limit = 5, bool $all = false, int $timeframe = 0 ): bool {
			...
			if ( $all ) {
				...
				$r    = $this->processAllData( $table_name, $data );
				if ( $r ) {
					$this->updateSyncTime( 'publishers' );
				}
			} else {
				$this->debugLog( "last sync was " . $lastSync );
		...
		public function updateReportTypesDetails() {
			...
			foreach ( $reportTypes as $type ) {
				...
				if ( ! $r ) {
					$this->debugLog( 'Error inserting for ' . $type->id . ' for gateway 1' );
				}
				...
				$r           = $wpdb->replace( $table_name, $insert_data );
				if ( ! $r ) {
					$this->debugLog( 'Error inserting for ' . $type->id . ' for gateway 2' );
				}
				...
				$r           = $wpdb->replace( $table_name, $insert_data );
				if ( ! $r ) {
					$this->debugLog( 'Error inserting for ' . $type->id . ' for gateway 3' );
				}
			}
		}
		...
		public function reportIntervals() {
			...
			foreach ( $results as $r ) {
				...
				if ( ! isset($return[ $r->id ][$r->gateway] ) ) $return[ $r->id ][$r->gateway]=[];
